  ...
  describe "#rooms_for_full_recording_sync" do
    ...
    context "returns a query to find rooms that had meetings in the past 7 days" do
      ...
      let!(:meeting1) {
        ...
        FactoryBot.create(:bigbluebutton_meeting, room: room1, create_time: create_time)
      }

      let!(:room2) { FactoryBot.create(:bigbluebutton_room) }
      let!(:meeting2) {
        create_time = (DateTime.now - 1.day).to_i * 1000
        FactoryBot.create(:bigbluebutton_meeting, room: room2, create_time: create_time)
      }

      let!(:room3) { FactoryBot.create(:bigbluebutton_room) }
      let!(:meeting3) {
        create_time = (DateTime.now - 7.days).to_i * 1000
        FactoryBot.create(:bigbluebutton_meeting, room: room3, create_time: create_time)
      }

      let!(:room4) { FactoryBot.create(:bigbluebutton_room) }
      let!(:meeting4) {
        create_time = (DateTime.now - 7.days - 1.second).to_i * 1000
        FactoryBot.create(:bigbluebutton_meeting, room: room4, create_time: create_time)
      }

      let!(:room5) { FactoryBot.create(:bigbluebutton_room) }
      let!(:meeting5) {
        create_time = (DateTime.now - 8.days).to_i * 1000
        FactoryBot.create(:bigbluebutton_meeting, room: room5, create_time: create_time)
      }

...
class Bigbluebutton::RoomsController < ApplicationController
  ...
  def room_allowed_params
    [ :name, :server_id, :meetingid, :attendee_key, :moderator_key, :welcome_msg,
      ...
      :auto_start_video, :auto_start_audio, :moderator_only_message, :auto_start_recording,
      :allow_start_stop_recording, :metadata_attributes => [ :id, :name, :content, :_destroy, :owner_id ] ]
  end
end

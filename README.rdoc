...
user has in the <tt>room</tt>, respectively.

These methods are defined in <tt>lib/bigbluebutton_rails/controller_methods.rb</tt> and you can reimplement them in your application_controller to change their behaviour.
...
These files are used by the views <tt>join_wait</tt> and <tt>invite</tt> and can be copied into your application with:
...
The server controller has basically only the RESTful actions defined in Rails.

The rooms controller also have the RESTful actions, plus specific actions to "join", "end", and check if a meeting is being held currently in the room ("running").
...
* If the user is a normal attendee:
  ...
  * Otherwise, redirects to <tt>join_wait</tt>, to wait for a moderator before joining the conference.
...
data from it, while the send methods just make the API call.

You can, for exemple, use <tt>fetch_meeting_info</tt> to get information about the meeting (API call <tt>getMeetingInfo</tt>). The data parsed from the
response wil be stored in the model and can be accessed using it's attributes. 

All methods in the models that trigger any API call are documented with the phrase "Triggers API call" and the API functions that are called. Be aware that they

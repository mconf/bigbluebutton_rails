...
* 0.81: Altough not all features are supported yet. See {TODO_08}[https://github.com/mconf/bigbluebutton_rails/blob/master/TODO_08].
...
is detected that a user joined a room and that he's the first user. Meetings are never removed, they are kept as a registry of
...
To keep track of meetings, you have to run the resque workers (in development and production):

  rake resque:work QUEUE='bigbluebutton_rails'

The list of meetigns is also periodically synchronized using cron tasks, as done for the list of recordings (see the section above).
